...
use App\Actions\PaymentPlan\UpdatePaymentPlan;
...
class EditPaymentPlanForm extends Component
{
    ...
    /**
     * Atualiza o plano de pagamento
     *
     * @param UpdatePaymentPlan $updater
     * @param bool $redirectAfterUpdate
     */
    public function updatePaymentPlan(UpdatePaymentPlan $updater, $redirectAfterUpdate = true)
    {
        $this->state['installment_indexes'] = $this->installmentIndexes->toArray();
        $updater->update($this->plan, $this->state);

        $this->successAction('Plano de pagamento atualizado.', ['payment-plans.index'], $redirectAfterUpdate);
    }
