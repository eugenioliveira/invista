...
use App\Actions\PaymentPlan\CreateNewPaymentPlan;
use App\Models\PaymentPlan;
use Illuminate\Support\Collection;
use Livewire\Component;
use Validator;

class CreatePaymentPlanForm extends Component
{
    use RedirectHandler;
    ...
    {
        $this->plan = new PaymentPlan();
        $this->installmentIndexes = new Collection();
    }
    ...
    public function createPaymentPlan(CreateNewPaymentPlan $creator, $redirectAfterCreate = true)
    {
        $this->state['installment_indexes'] = $this->installmentIndexes->toArray();
        $creator->create($this->state);

        $this->successAction('Plano de pagamento criado com sucesso!', ['payment-plans.index'], $redirectAfterCreate);
    }
    ...
    {
        return view('livewire.create-payment-plan-form', [
