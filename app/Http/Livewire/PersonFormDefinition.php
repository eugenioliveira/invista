...
trait PersonFormDefinition
{
     ...
     * @var array
     */
    public array $state = ['birth' => '', 'partner_cpf' => ''];
    ...
    {
        return [
            'person.firstname' => ['required', 'min:3'],
            'person.lastname' => ['required', 'min:2'],
    ...
    public function messages(): array
    {
        return [
            ...
            'person.firstname.min' => 'O campo primeiro nome deve conter no mínimo 3 caracteres.',
            'person.lastname.required' => 'O campo sobrenome é obrigatório.',
            'person.lastname.min' => 'O campo sobrenome deve conter no mínimo 2 caracteres.',
    ...
    /**
     * Valida se o CPF é único na base de dados.
     */
    public function validateCpfUniqueness(): bool
    {
        $this->resetErrorBag();
        ...
        if ($cpfValidator->fails()) {
            $this->addError('person.cpf', $cpfValidator->errors()->first('cpf'));
            return false;
        }
    ...
    /**
     * Salva a pessoa.
     * @param $redirectAfterSave
     * @throws \Exception
     */
    public function savePerson($redirectAfterSave = true)
    {
        $this->resetErrorBag();

        if ($this->validateCpfUniqueness()) {
            $this->validate();
            ...
            try {
                ...
                if($this->partner->id) {
                    $this->person->detail->partner_id = $this->partner->id;
                    $this->person->detail->save();
                } else {
                    $this->person->detail->becomeSingle();
                }
                DB::commit();

                // Redireciona
                $this->successAction('Pessoa salva!', ['people.index'], $redirectAfterSave);
            } catch (\Exception $e) {
                DB::rollBack();
                throw $e;
            }
